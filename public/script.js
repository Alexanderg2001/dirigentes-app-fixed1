...
function filtrarDirigentes() {
    const searchTerm = document.getElementById('buscar-dirigente').value.toLowerCase();
    const filtroCorregimiento = document.getElementById('filtro-corregimiento').value;
    const filtroParticipacion = document.getElementById('filtro-participacion').value;
    ...
    for (let row of rows) {
        ...
        const corregimiento = row.cells[3].textContent;
        const comunidad = row.cells[4].textContent.toLowerCase();
        const coordinador = row.cells[5].textContent.toLowerCase();
        const participacion = row.cells[6].textContent.toLowerCase();
        
        const coincideBusqueda = nombre.includes(searchTerm) || 
                               cedula.includes(searchTerm) || 
                               telefono.includes(searchTerm) ||
                               comunidad.includes(searchTerm) ||
                               coordinador.includes(searchTerm);
        
        const coincideCorregimiento = !filtroCorregimiento || corregimiento === filtroCorregimiento;
        const coincideParticipacion = !filtroParticipacion || participacion === filtroParticipacion;
        
        if (coincideBusqueda && coincideCorregimiento && coincideParticipacion) {
            row.style.display = '';
            resultadosVisibles++;
        } else {
    ...
    if (resultadosVisibles === 0) {
        if (!noResults) {
            const tr = document.createElement('tr');
            tr.id = 'no-results';
            tr.innerHTML = `<td colspan="8" class="no-results">No se encontraron dirigentes con los filtros aplicados</td>`;
            tbody.appendChild(tr);
        }
    } else if (noResults) {
        noResults.remove();
